...
public class ProdWebAdminLoginController {
	@FXML
	private Button btnProdLaunchWebAdmin=null;
	@FXML
	...
	private Stage getParentStage() {
		if(parentStage == null) {
			parentStage = (Stage)btnProdLaunchWebAdmin.getScene().getWindow();
		}
		return parentStage;
	}
	
	@FXML
	public void btnProdLaunchWebAdminClicked(Event e) {
		ControllerUtils.hideStages(getParentStage());
		driver = getDriver();
		driver.get(ProdSettings.WEBADMIN_LINK);
		ControllerUtils.showStages(getParentStage());
		btnProdLaunchWebAdmin.getStyleClass().add("success");
	}
	...
	public void btnProdResetPinClicked(Event e) {
		...
		boolean ret = resetPin("qtpicuser" , ProdSettings.ENROLLMENT_USERNAME, "test1234");
		if (ret==false) {
